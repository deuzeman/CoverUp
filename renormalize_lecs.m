function data = renormalize_lecs(params, data)
    ...
    data.lbar1_n = params.l1 + 2 * log(almanac.mpi) - log(data.chimu_n);

    data.lbar2_c = params.l2 + 2 * log(almanac.mpi) - log(data.chimu_c);
    data.lbar2_n = params.l2 + 2 * log(almanac.mpi) - log(data.chimu_n);

    data.lbar3_c = params.l3 + 2 * log(almanac.mpi) - log(data.chimu_c);
    data.lbar3_n = params.l3 + 2 * log(almanac.mpi) - log(data.chimu_n);

    data.lbar4_c = params.l4 + 2 * log(almanac.mpi) - log(data.chimu_c);
    data.lbar4_n = params.l4 + 2 * log(almanac.mpi) - log(data.chimu_n);
    
    if data.has_iso
        data.Xibar3_n = params.Xi3 + 2 * log(almanac.mpi) - log(data.chimu_n);
    end
end
